...
parser.add_argument('--resume_iter', type=int, default=0,
                    help='Iteration number to resume from (e.g. 11200)')
...
def train_one_round_mt(args, round_num, snapshot_path, pseudo_labels, image_list, writer,
    ...
    if resume_ckpt and resume_iter > 0:
        logging.info("Resuming Round %d from iter %d, loading %s", round_num, resume_iter, resume_ckpt)
        state_dict = torch.load(resume_ckpt, map_location='cuda', weights_only=False)
        model.load_state_dict(state_dict)
        ema_model.load_state_dict(state_dict)
        # Parse best dice from checkpoint filename (e.g. round2_iter11200_dice0.6716.pth)
        import re
        dice_match = re.search(r'dice(\d+\.\d+)', os.path.basename(resume_ckpt))
        best_performance = float(dice_match.group(1)) if dice_match else 0.0
        logging.info("Resumed best_performance = %.4f from checkpoint filename", best_performance)
    else:
        resume_iter = 0
    ...
    # Set initial learning rate for resumed iteration
    if resume_iter > 0:
        lr_ = args.base_lr * (1.0 - resume_iter / max_iterations) ** 0.9
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr_
        logging.info("Resumed learning rate = %.6f", lr_)

    iterator = tqdm(range(max_epoch), ncols=70, desc=f"Round {round_num}")
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            # Skip batches until we reach the resume iteration
            if iter_num < resume_iter:
                iter_num += 1
                continue
...
# ---- UAMT ----
def train_one_round_uamt(args, round_num, snapshot_path, pseudo_labels, image_list, writer, **kwargs):
    num_classes = args.num_classes
    max_iterations = args.max_iterations
...
# ---- DAN ----
def train_one_round_dan(args, round_num, snapshot_path, pseudo_labels, image_list, writer, **kwargs):
...
# ---- DTC ----
def train_one_round_dtc(args, round_num, snapshot_path, pseudo_labels, image_list, writer, **kwargs):
    num_classes = args.num_classes
    max_iterations = args.max_iterations
...
def main():
    ...
    for round_num in range(1, args.num_rounds + 1):
        ...
        # Skip completed rounds when resuming
        if resume_round > 0 and round_num < resume_round:
            logging.info("Skipping Round %d / %d (resuming from Round %d)",
                         round_num, args.num_rounds, resume_round)
            # Still need to regenerate pseudo-labels for the next round
            # Load the best model from this completed round to generate them
            best_ckpt_path = os.path.join(snapshot_path, f'round{round_num}_best.pth')
            if round_num < args.num_rounds:
                logging.info("Loading Round %d best model to regenerate pseudo-labels", round_num)
                skip_model = net_factory_3d(net_type=args.model, in_chns=1,
                                            class_num=args.num_classes).cuda()
                skip_model.load_state_dict(
                    torch.load(best_ckpt_path, map_location='cuda', weights_only=False))
                new_pseudo_labels, uncertainties = generate_pseudo_labels_from_model(
                    skip_model, args.root_path, image_list, args.labeled_num,
                    num_classes=args.num_classes, patch_size=args.patch_size,
        ...
        # Pass resume info only for the specific round being resumed
        if resume_round > 0 and round_num == resume_round and resume_ckpt:
            model, best_dice = train_fn(
                args, round_num, snapshot_path, pseudo_labels, image_list, writer,
                resume_ckpt=resume_ckpt, resume_iter=resume_iter)
        else:
            model, best_dice = train_fn(
                args, round_num, snapshot_path, pseudo_labels, image_list, writer)
